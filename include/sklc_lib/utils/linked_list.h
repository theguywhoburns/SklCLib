...
typedef struct linked_list {
    ...
    struct Node* Last;
    uint64_t count;
    void (*AddLast)     (struct linked_list* list, void* value, uint64_t size);
    void (*AddFirst)    (struct linked_list* list, void* value, uint64_t size);
    void (*AddLastNode) (struct linked_list* list, struct Node* node);
    void (*AddFirstNode)(struct linked_list* list, struct Node* node);
    uint64_t  (*Count)    (struct linked_list* list);
    //use only if you dont have manually allocated variables in nodes
    void (*Clear)    (struct linked_list* list);
    void*(*GetAt)    (struct linked_list* list, uint64_t id);
    struct Node*(*GetNodeAt)(struct linked_list* list, uint64_t id);
} linked_list;
