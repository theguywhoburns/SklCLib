...
void* _darray_create(uint64_t length, uint64_t stride) {
    uint64_t header_size = DARRAY_FILED_LENGTH * sizeof(uint64_t);
    ...
    return (void*)(new_array + DARRAY_FILED_LENGTH);
}
void _darray_destroy(void* array) {
    uint64_t* header = (uint64_t*) array - DARRAY_FILED_LENGTH;
    //uint64_t header_size = DARRAY_FILED_LENGTH * sizeof(uint64_t);
...
void* _darray_pop_at(void* array, uint64_t index, void* dest) {
    ...
    uint64_t stride = darray_stride(array);
    if(index >= length) {
        printf("ERROR: Index outside the bounds of this array! Length:%llu, index %llu", length, index);
        return array;
    }
    ...
    // if not on the last element, strip out the netry and copy the rest inward
    if(index != length -1) {
        memcpy(
...
void* _darray_insert_at(void* array, uint64_t index, void* value_ptr) {
    ...
    // if not on the lats element, copy the rest outward
    if(index != length -1) {
        memcpy(
            (void*)(addr + ((index + 1) * stride)),
            (void*)(addr + (index * stride)),
